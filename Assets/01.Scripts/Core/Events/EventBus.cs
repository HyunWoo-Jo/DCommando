...
namespace Game.Core.Event
{
    public class EventBus : IEventBus, IDisposable {
        ...
        public void Publish<T>(T eventData) where T : class {
            if (eventData == null) {
                Debug.LogWarning($"이벤트 데이터가 null입니다: {typeof(T).Name}");
                return;
            }
            ...
            if (_subjects.TryGetValue(eventType, out var subject)) {
                if (subject is Subject<T> typedSubject) {
                    ...
                    Debug.Log($"이벤트 발행: {eventType.Name}");
                }
            } else {
                Debug.LogWarning($"구독자가 없는 이벤트입니다: {eventType.Name}");
            }
        }
        ...
        public IDisposable Subscribe<T>(Action<T> onEvent) where T : class {
            if (onEvent == null) {
                Debug.LogError("이벤트 핸들러가 null입니다.");
            ...
            Debug.Log($"이벤트 구독: {typeof(T).Name}");
            return subscription;
        }
        ...
        private Subject<T> GetOrCreateSubject<T>() where T : class {
            ...
            if (!_subjects.TryGetValue(eventType, out var subject)) {
                ...
                Debug.Log($"새로운 이벤트 스트림 생성: {eventType.Name}");
                return newSubject;
            }
